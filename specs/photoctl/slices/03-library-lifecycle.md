...
**Artifact invariant:** backup remains deliberately metadata-only. Restore preserves existing canonical artifacts,
library-owned originals, previews, presets and backups while rebuilding catalog state. Restored graph metadata can
still point at an artifact already lost outside the database and report `artifact_available:false`; the SQL snapshot
does not contain or reproduce paid/generated pixels. The restore owner defines the preservation and crash-recovery path.
...
- **3b** `packages/library/src/backup.ts`: `photoctl backup` → `<lib>/backups/<iso>.sql` via `@electric-sql/pglite-tools` `pgDump`;
  ...
  Library-owned file trees survive the catalog replacement; no compatibility path or full-artifact backup mode is added.
