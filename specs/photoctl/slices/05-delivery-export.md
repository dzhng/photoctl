...
- Export resolves the canonical `render_hash` at command start, renders from that immutable state snapshot, and includes
  `render_hash` in every successful item result. A concurrently committed edit gets a new hash and cannot partially enter the export.
