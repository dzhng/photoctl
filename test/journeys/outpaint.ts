...
export function registerOutpaintJourney(
  ...
  test.each([
    { label: "cropped-away support", cropX: 20, retouchX: 18 },
    { label: "outside original support", cropX: 0, retouchX: -2 },
  ])(
    `${title}: $label`,
    async ({ label, cropX, retouchX }) => {
      ...
      try {
        ...
        const snapshotHandle = await openLibrary(library, { noDaemon: true });
        try {
          await snapshotHandle.query(
        ...
        const exported = async (name: string) => {
          ...
          if (capture) {
            const destination = join(capture, label);
            await mkdir(destination, { recursive: true });
            await copyFile(file, join(destination, `${name}.png`));
          }
        ...
        const retouched = retouchDataSchema.parse(
          (await run(["retouch", id, "--at", `${retouchX},34`, "--radius", "1"])).data,
        );
        const normalized = retouchDataSchema.parse(
          (
            await run([
              "retouch",
              id,
        ...
        expect(normalized).toEqual({ ...retouched, reused: true });
        const revisionSnapshot = async () => {
          ...
          try {
            return (
              ...
              (SELECT count(*)::text FROM image_nodes WHERE photo_id = $1) AS nodes
             FROM photo_documents WHERE photo_id = $1`,
                [id],
              )
            ).rows;
          } finally {
        ...
        const beforeInvalid = await revisionSnapshot();
        const rejectGeometry = async (args: string[]) => {
          const rejected = await invoke(["retouch", id, ...args], { libraryDir: library, env });
          expect(rejected.code).toBe(2);
          expect(rejected.json).toMatchObject({ ok: false, code: "usage" });
        };
        await rejectGeometry(["--at", "-1000,34", "--radius", "1"]);
        await rejectGeometry(["--at", `${retouchX},34`, "--radius", "0.00001"]);
        await rejectGeometry(["--at", `${retouchX},34`, "--radius", "1000"]);
        expect(await revisionSnapshot()).toEqual(beforeInvalid);
