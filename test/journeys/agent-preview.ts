...
export function registerAgentPreviewJourney(
  invoke: typeof spawnPhotoctl = spawnPhotoctl,
  title = "the built CLI keeps agent previews current through fill, opacity, retouch, generative, and markup revisions",
) {
  ...
  afterEach(async () => {
    ...
    server = undefined;
    await Promise.all(
      directories.splice(0).map(async (path) => await rm(path, { recursive: true })),
    );
  });
  ...
  test(title, async () => {
    ...
    do {
      ...
      expect(page.code, JSON.stringify(page.json)).toBe(0);
      const data = graphShowDataSchema.parse(page.json.data);
      expect(data.render_hash).toBe(h2);
      expect(data.revision_id).toBe(fullGraph.revision_id);
      expect(data.roots).toEqual(fullGraph.roots);
      expect(data.nodes.length).toBeLessThanOrEqual(2);
      pagedNodes.push(...data.nodes);
      cursor = data.next_cursor ?? undefined;
      if (cursor) {
        expect(cursors.has(cursor)).toBe(false);
        cursors.add(cursor);
        expect(cursors.size).toBeLessThanOrEqual(graphNodes.length);
      }
    } while (cursor);
    expect(cursors.size).toBeGreaterThan(0);
    expect(pagedNodes).toEqual(graphNodes);
    for (const nodeId of [fillData.generation.node, fillData.upscale.node]) {
      const inspected = await run(fixture, ["graph", "node", fixture.id, nodeId]);
      expect(inspected.code, JSON.stringify(inspected.json)).toBe(0);
      const node = graphNodeDataSchema.parse(inspected.json.data);
      expect(node.id).toBe(nodeId);
      expect(node.record_truncated).toBe(false);
      expect(node.parameters_truncated).toBe(false);
      expect(node.executions.length).toBeGreaterThan(0);
      for (const execution of node.executions) {
        expect(execution.artifact_available).toBe(true);
        expect(execution.provider_provenance).toMatchObject({
          parameters_truncated: false,
          execution_id: execution.execution_id,
          output: { artifact_hash: execution.output_artifact_hash, available: true },
        });
      }
    }
    ...
    const removedReimagine = await run(fixture, [
      "layer",
      "remove",
      fixture.id,
      reimagine.layer_id,
  ...
  async function setupFixture() {
    ...
    expect((await invoke(["init", "--path", library], { env })).code).toBe(0);
    const unconfigured = await invoke(["doctor"], { libraryDir: library, env });
    expect(unconfigured.code, JSON.stringify(unconfigured.json)).toBe(0);
    expect(unconfigured.json).toMatchObject({
      data: { providers: { upscale: { configured: false } } },
    });
  ...
  async function run(fixture: Fixture, args: string[]) {
    return await invoke(args, { libraryDir: fixture.library, env: fixture.env });
  }
