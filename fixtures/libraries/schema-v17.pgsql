...
CREATE TABLE public.image_nodes (
    ...
    CONSTRAINT image_nodes_recipe_version_check CHECK ((((kind = ANY (ARRAY['composite'::text, 'resample'::text, 'generate'::text, 'mask'::text])) AND (recipe_version = ANY (ARRAY[1, 2]))) OR ((kind <> ALL (ARRAY['composite'::text, 'resample'::text, 'generate'::text, 'mask'::text])) AND (recipe_version = 1))))
);
