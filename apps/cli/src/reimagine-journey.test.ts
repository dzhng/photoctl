...
test("the built CLI reimagines lazily at base density and removal restores exact prior pixels", async () => {
  const directory = await mkdtemp(join(tmpdir(), "photoctl-reimagine-journey-"));
  ...
  const mutation = await spawnPhotoctl(
    ["reimagine", id, "--prompt", "painted twilight", "--strength", "0.5"],
    { libraryDir: library, env },
  );
  expect(mutation.code, JSON.stringify(mutation.json)).toBe(0);
  expect(mutation.events).toEqual(
    expect.arrayContaining([
      { event: "progress", phase: "reimagine", done: 0, total: 1 },
      { event: "progress", phase: "reimagine", done: 1, total: 1 },
    ]),
  );
  const changed = reimagineDataSchema.parse(mutation.json.data);
  ...
  expect(String(requests[0]!.fields.prompt)).toContain("[photoctl:reimagine:v1]");
  expect(String(requests[0]!.fields.prompt)).toContain("preserving approximately 50%");

  const listed = await spawnPhotoctl(["layer", "list", id], { libraryDir: library, env });
  expect(listed.json).toMatchObject({
    data: { layers: [{ id: changed.layer_id, role: "reimagine", z: 0 }] },
  });
  const inspected = await spawnPhotoctl(["layer", "show", id, changed.layer_id], {
    libraryDir: library,
    env,
  });
  expect(
...
async function capture(files: Record<string, string>) {
  const destination = process.env.PHOTOCTL_REIMAGINE_CAPTURE_DIR;
