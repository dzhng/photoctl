  ...
  const cards = evidence.photos
    .map(({ row, preview, show }) => {
      ...
      const originals = row.originals
        .map(
          (original) =>
            `<span class="original" data-online="${original.online}" title="${escapeHtml(original.id)}"><i></i>${original.kind.toUpperCase()}${original.id === row.primary_original_id ? " · Primary" : ""} · ${original.online ? "Online" : "Offline"}</span>`,
        )
        .join("");
      return `<article class="photo" data-online="${row.online}">
        ...
        <div class="meta"><div class="title"><h2>${escapeHtml(row.file)}</h2><span class="online"><i></i>${row.online ? "Primary online" : "Primary offline"}</span></div><p class="id">${escapeHtml(row.id)}</p>
          <div class="badges"><span class="rating">${stars}</span><span class="flag ${row.flag}">${flag}</span><span class="label ${row.label ?? "none"}">${label}</span></div>
          <div class="originals"><span class="originals-label">Originals</span>${originals}</div>
