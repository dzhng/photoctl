...
const PIXELS_OFFSET = SAMPLE_FORMAT_OFFSET + 6;
...
export async function encodeLinearTiff(image: LinearImage): Promise<Buffer> {
  ...
  const output = Buffer.allocUnsafe(PIXELS_OFFSET + pixelBytes);
  ...
  entry = writeIfdEntry(output, entry, 273, 4, 1, PIXELS_OFFSET);
  ...
  for (let index = 0; index < image.data.length; index += 1) {
    const value =
      image.space === "camera" ? (image.data[index] - image.blackLevel) / range : image.data[index];
    const sample = Math.round(Math.max(0, Math.min(1, value)) * 65_535);
    output.writeUInt16LE(sample, PIXELS_OFFSET + index * 2);
  }
  return output;
}
