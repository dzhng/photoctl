...
import { decodeExternalImage, image16Png } from "./external-pixels.js";
...
export async function executeStandaloneGeneration(
  ...
  return (
    await runProviderImageAttempt<PreparedGeneration>(
      database,
      libraryPath,
      {
        operation: "generate",
      ...
      async (attempt) => {
        ...
        return {
          ...
          artifacts: [
            artifact,
            ...(reference ? [reference.artifact, reference.encodedArtifact] : []),
          ],
          executions: [
            {
    ...
    await runProviderImageAttempt<PreparedGeneration>(
      ...
      async (attempt) => {
        ...
        const parameters = {
          ...
          prompt_version: input.promptVersion,
          request: {
            ...(input.request(executionId, normalized.returnedDimensions) as Record<
              string,
              JsonValue
            >),
            ...(reference
  ...
  try {
    ...
    const upscaleOperation = densityPlan?.upscale.operations.find(
      (
        ...
        (typeof densityPlan.upscale.operations)[number],
        { kind: "upscale" }
    ...
    if (
      ...
      const executionId = newExecutionId();
      const result = await executeRetainedUpscale(database, libraryPath, input.upscale.adapter, {
        artifact: {
          ...
          appendWarnings(warnings, [
            {
              code: "upscale_failed",
              message:
                error instanceof Error ? error.message : "Upscaler returned unreadable pixels",
            },
          ]);
    ...
    try {
      ...
      /* Preserve the primary diagnostic if the catalog itself became unavailable. */
    }
    throw error;
  }
}
