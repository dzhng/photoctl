...
describe.each(["openai/gpt-image-2.5-flare", "openai/gpt-image-2.5-sunburst"])(
  ...
  (model) => {
    test("a coprime crop uses uniform scaling and protected padding with an explicit inverse map", async () => {
      ...
      expect(prepared.frameMapping).toEqual({
        source: [0, 0, 383, 384],
        output: [0, 0, 1149, 1152],
      });
    ...
    test.each([
      ...
      const prepared = createGatewayImageModelAdapter({ model }).buildGeneration("a landscape", {
        w,
        h,
      });
    ...
    test("fixture model raster sizes are independent of the real model policy", () => {
      const prepared = createGatewayImageModelAdapter({
        model: FAKE_IMAGE_EDIT_MODEL,
      }).buildGeneration("a landscape", { w: 383, h: 384 });
    ...
    test.each(["generation", "reference generation", "full-frame edit"])(
      ...
      async (operation) => {
        ...
        const size =
          prepared.body instanceof FormData ? prepared.body.get("size") : prepared.body.size;
    ...
    test.each([
      ...
      expect(() => adapter.buildGeneration("a landscape", { w, h })).toThrow(
        `No supported ${model} canvas preserves ${w}x${h}`,
      );
    });
    ...
    test("standalone composition is explicitly confined to its requested aspect inside the provider canvas", () => {
      ...
      expect(prepared.frameMapping).toEqual({
        source: [0, 0, 1001, 1000],
        output: [0, 0, 1001, 1000],
      });
